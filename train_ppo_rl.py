"""
train_ppo_rl.py — Train PPO on BrickEnvRL (pure RL, no pre-defined plan)
=========================================================================
...
Usage
-----
python3 train_ppo_rl.py --spec lineSpecifications/spec1.yaml
python3 train_ppo_rl.py --spec lineSpecifications/spec3.yaml --timesteps 1000000 --n_envs 8
python3 train_ppo_rl.py --spec lineSpecifications/spec1.yaml --load_model models/ppo_rl/checkpoints/ppo_rl_brick_10000_steps

Note: this task is harder than the plan-guided version — expect to need
     more timesteps to reach comparable performance.
"""
...
# ---------------------------------------------------------------------------
# Rollout CSV callback  (identical logic to train_ppo.py)
# ---------------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train PPO on BrickEnvRL (pure RL — no plan)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--spec",       type=str, default="lineSpecifications/spec1.yaml")
    p.add_argument("--timesteps",  type=int, default=500_000,
                   help="Total environment steps (needs more than plan-guided version)")
    p.add_argument("--n_envs",     type=int, default=4)
    p.add_argument("--save_dir",   type=str, default="models/ppo_rl")
...
def main() -> None:
    ...
    rollout_csv = os.path.join(rollout_dir, f"ppo_rl_{spec_label}_rollout.csv")
    ...
    if args.load_model:
        ...
        model = PPO(
            ...
            gae_lambda      = 0.95,
            clip_range      = 0.2,
            ent_coef        = 0.05,   # higher entropy — more exploration needed
            verbose         = 1,
            seed            = args.seed,
            tensorboard_log = tb_dir,
        )
    ...
    # ---- test episode -------------------------------------------------------
    run_test_episode(model, spec, algo_label="PPO")
