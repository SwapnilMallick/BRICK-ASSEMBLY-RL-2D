...
spec = importlib.util.spec_from_file_location(
    "brick2denv",
    os.path.join(os.path.dirname(__file__), "2dEnv", "2dEnv_obj_v2.py"),
)
...
def choose_valid_target(env, orientation: float = 0.0) -> (float, float) or None:
    ...
    # final check
    if env._check_valid_placement(target_x, target_y, orientation):
...
def _project_point_to_segment(px: float, py: float, ax: float, ay: float, bx: float, by: float) -> Tuple[float, float]:
    """Project a point to the closest point on a line segment."""
    abx = bx - ax
    aby = by - ay
    ab2 = abx * abx + aby * aby
    if ab2 <= 1e-12:
...
def snap_target_to_wall(env, x: float, y: float, orientation: float) -> Tuple[float, float]:
    """
    Snap planned target onto the closest wall segment that matches orientation.
    This removes perpendicular offsets that show up as visual gaps on diagonals.
    """
    best = None
    for seg in env.lineFigure:
        ax, ay = float(seg["start"][0]), float(seg["start"][1])
        bx, by = float(seg["end"][0]), float(seg["end"][1])
        seg_angle = float(np.degrees(np.arctan2(by - ay, bx - ax)) % 360.0)

        # Treat both directions on the same line as valid matches.
        orientation_mismatch = min(
            _angle_diff_deg(orientation, seg_angle),
            _angle_diff_deg((orientation + 180.0) % 360.0, seg_angle),
        )
        if orientation_mismatch > 20.0:
            continue

        sx, sy = _project_point_to_segment(x, y, ax, ay, bx, by)
        dist2 = (x - sx) ** 2 + (y - sy) ** 2
        score = (dist2, orientation_mismatch)
        if best is None or score < best[0]:
            best = (score, sx, sy)

    # Fallback: use nearest segment projection if no orientation-matching segment was found.
    if best is None:
        for seg in env.lineFigure:
            ax, ay = float(seg["start"][0]), float(seg["start"][1])
            bx, by = float(seg["end"][0]), float(seg["end"][1])
            sx, sy = _project_point_to_segment(x, y, ax, ay, bx, by)
            dist2 = (x - sx) ** 2 + (y - sy) ** 2
            if best is None or dist2 < best[0][0]:
                best = ((dist2, 999.0), sx, sy)

    snapped_x, snapped_y = float(best[1]), float(best[2])

    # Preserve intentional offsets (e.g., "outer side" plans).
    # Only snap when target is already near the wall.
    distance = float(np.hypot(x - snapped_x, y - snapped_y))
    near_wall_tol = max(1.0, float(getattr(env, "lineFigure", [{}])[0].get("thickness", 1.0)) + 0.25)
    if distance > near_wall_tol:
...
def main():
    ...
    for entry in placement_list:
        ...
        if use_plan and "x" in entry and "y" in entry:
            ...
            snapped_x, snapped_y = snap_target_to_wall(env, target_x, target_y, orientation)
            target = (snapped_x, snapped_y)
            print(
                f"Placing at planned location: ({target_x:.4f}, {target_y:.4f}) "
                f"-> snapped to wall: ({snapped_x:.4f}, {snapped_y:.4f})"
            )
        else:
            # choose a valid target; retry a few times if needed
            target = None
            for _ in range(20):
                candidate = choose_valid_target(env, orientation)
        ...
        # Validate placement before animation. If invalid (including overlap), apply penalty and discard.
        if not env._check_valid_placement(target_x, target_y, orientation):
            place_action = {
            ...
            print(
                f"Brick {brickID}: invalid placement at ({target_x:.4f}, {target_y:.4f}), "
                f"reward={reward:.3f}. Discarding brick visual."
            )
            env.render(mode="human")
            capture_frame(env, writer)
            time.sleep(0.08)
        ...
        if reward <= 0.0:
            print(
                f"Brick {brickID}: placement rejected at ({target_x:.4f}, {target_y:.4f}), "
                f"reward={reward:.3f}. Skipping visual."
            )
            env.render(mode="human")
            capture_frame(env, writer)
            time.sleep(0.08)
