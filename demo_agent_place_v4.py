...
def choose_valid_target(env) -> (float, float) or None:
    # Pick a random line segment and choose a position on it
    seg = random.choice(env.lineFigure)
    x_min = float(min(seg["start"][0], seg["end"][0]))
    x_max = float(max(seg["start"][0], seg["end"][0]))
    # ensure within brick length bounds
    x_max = min(x_max, env.x_dim - env.brick_length)
    x_min = max(0.0, x_min)
    if x_max < x_min:
        return None
...
def animate_move(env, start_px, end_px, width_px, height_px, placed_visuals, frames=20, color=(255,100,0), writer=None) -> None:
    ...
    for f in range(frames):
        ...
        # draw existing placed visuals (only orange bricks)
        for pv in placed_visuals:
            if pv.get("color") != (255, 100, 0):
                continue
            rect = pygame.Rect(int(pv["x"]), int(pv["y"]), int(pv["width"]), int(pv["height"]))
            pygame.draw.rect(env.screen, pv["color"], rect)
...
def _draw_rotated_bricks(screen, placed_visuals):
    for pv in placed_visuals:
        # Only draw orange bricks during visualization; skip red placed bricks
        if pv.get("color") != (255, 100, 0):
            continue
...
def main():
    ...
    total = env.total_bricks
    for brickID in range(total):
        # pick
        pick_action = {
